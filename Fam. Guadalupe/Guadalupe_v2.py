...
st.markdown(f"""
    ...
    /* CELULAR (≤ 768px) - Animación reducida y optimizada */
    @media (max-width: 768px) {{
        [data-testid="stAppViewContainer"] {{
            ...
            animation: recorridoPanoramicoCelular 20s ease-in-out infinite alternate !important;
        }}
        
        @keyframes recorridoPanoramicoCelular {{
            0% {{ background-position: -30% center !important; }}
            100% {{ background-position: 130% center !important; }}
        }}
    }}
    ...
    /* CELULAR - Reposicionado sin centrado */
    @media (max-width: 768px) {{
        .mini-logo-flotante-master {{
            position: fixed !important;
            top: 70px !important;
...
# ============================================================================
    ...
    if st.session_state.pantalla_actual == "bienvenida":
        ...
        # Logotipo con destello metálico
        if URL_LOGO_PORTADA:
            st.markdown(f"""
